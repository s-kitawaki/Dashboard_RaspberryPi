...
SVGのドット絵のキャラクターが2か所にいます。いずれも足踏み・まばたきを行い、21:00〜翌5:59はその場で眠り、OSの「視差効果を減らす」設定では動きを止めます。外部画像や追加APIは使用しません。

- **おへや（右下）**: くちぱっちといちごっちが住人です。床の帯の中をランダムな目的地へ上下左右に歩き、朝は歯みがきの定位置、寝室ではベッドの定位置で眠ります。
- **境目の散歩（2列レイアウト、幅900px以上）**: 日替わりで選ばれた2体が、上段と下段のパネルの境目の線を左から右へ歩き、カレンダーと為替レートの間の隙間と右端でジャンプします（1周30秒弱）。歩いている間は天気パネルの予報の行に体が重なります。幅899px以下では表示しません。

散歩役は日本時間の日付を種にした重み付き抽選で決まり、同じ日は何度読み込んでも同じ2体、日付が変わると入れ替わります（重複なし）。
...
各キャラクターは `src/lib/sprites.ts` に 40×40 の長方形の集まりとして定義しており、`TamaSprite.vue` が共通の動きで描きます。Storybook の **Dashboard → キャラクター → Gallery** で7体を一覧できます。
...
画面全体の背景は 256×150 ピクセルのドット絵を Canvas に描いて4倍表示したもので、日本時間の時間帯で部屋が切り替わります。切り替え時は1秒のクロスフェードで、OSの「視差効果を減らす」設定では即時に切り替わります。文字はすべてドットフォント DotGothic16（npm の `@fontsource/dotgothic16` を同梱、外部通信なし）です。

| 時間帯 | 部屋 | くちぱっち | おへやのキャラクター |
| --- | --- | --- | --- |
| 6:00〜10:59 | あさ・バスルーム | 歩く | 歯みがき |
| 11:00〜13:59 | ひる・リビング | 歩く | 床を歩き回る |
| 14:00〜16:59 | 夕方・リビング（夕焼け） | 歩く | 床を歩き回る |
| 17:00〜20:59 | よる・リビング（星空、電気とテレビ） | 歩く | テレビの前に寄りがち |
| 21:00〜翌5:59 | よる・寝室 | 眠る | 眠る |

パネルは「現在の天気」（気温・状態・直近3枠の予報）、「カレンダー」、「為替レート」の3枚で、右下はパネルを置かない「おへや」です。おへやのキャラクターは床の帯（`FLOOR_BAND`）の中でランダムな目的地へ上下左右に歩き、手前ほど少し大きく描きます。今は仮の1体だけで、キャラクターの追加は別作業です。Storybook では **Dashboard → 日々 → MorningRoom / DayRoom / EveningRoom / NightRoom / BedroomRoom** で各時間帯を、**Dashboard → おへや** でおへや単体を確認できます。
...
画面のブラウザーテストはサンプルデータを使い、1024×600のRaspberry Pi画面と390px幅のスマートフォン画面、毎時更新、日付変更、API失敗・再試行、5つの時間帯での部屋の切り替え、おへやのキャラクターが床の帯から出ないことを確認します。
