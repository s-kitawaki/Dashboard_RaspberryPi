...
SVGのドット絵のくちぱっちです。2列レイアウト（幅900px以上）では上段と下段のパネルの境目の線を左から右へ歩き、カレンダーと為替レートの間の隙間に来たらジャンプして飛び越え、右端まで来たらジャンプして左端に現れます（1周30秒弱）。歩いている間は天気パネルの予報の行に体が重なります。幅899px以下ではパネルが縦に積まれるため、時計の右の空き領域を約16秒で一往復します。足踏み・まばたきも行い、21:00〜翌5:59はその場で停止して睡眠になります。OSの「視差効果を減らす」設定では動きを止めます。外部画像や追加APIは使用しません。
